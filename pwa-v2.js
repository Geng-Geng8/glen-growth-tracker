(function GlenGrowthPWAV2() {
    ...
    function addHeadMetadata() {
        if (!document.querySelector('link[rel="manifest"]')) {
            ...
            manifest.href = './manifest.webmanifest?v=20260907-2';
        ...
        if (!document.querySelector('link[rel="apple-touch-icon"]')) {
            ...
            icon.href = './icon.svg?v=20260907-2';
    ...
    function installCachedLoaders() {
        window.loadTodayState = async function () {
        ...
        window.loadWeekState = async function () {
        ...
        if (typeof window.renderMoney === 'function') {
            window.loadMoneyState = async function () {
        ...
        if (typeof window.renderWins === 'function') {
            window.loadWinsState = async function () {
    ...
    function injectStyles() {
        if (document.getElementById('glen-growth-pwa-v2-styles')) return;
        const style = document.createElement('style');
        style.id = 'glen-growth-pwa-v2-styles';
        style.textContent = `
            ...
            @media (max-width: 340px) {
                .pwa-status-bar { align-items: flex-start; flex-direction: column; }
            }
        `;
    ...
    async function registerServiceWorker() {
        ...
        try {
            const registration = await navigator.serviceWorker.register('./sw-v2.js?v=20260907-1', { scope: './' });
